...
[.채팅수집동의거부] 이 계정의 `채팅 내역 수집` 동의를 철회합니다.
    ...
    def __init__(self, storage_instance):
        ...
        handlers = [self.add_file_handler("yrumee-{}.log".format(int(time.time()))), logging.StreamHandler()]
    ...
    async def on_command(self, command: str, payload: str, message: discord.Message):
        ...
        if command.lower() == "채팅수집동의":
            ...
            await message.author.send("[채팅수집-동의] {}님의 채팅내역 수집 동의가 정상적으로 처리되었습니다.".format(name))
        elif command.lower() == "채팅수집동의내역":
            status = "동의하였습니다" if self.agree_status.get(by, False) else "동의하지 않으셨습니다"
            await message.author.send("[채팅수집-동의내역] {}님은 채팅내역 수집에 {}.".format(name, status))
        elif command.lower() == "채팅수집동의거부":
            self.agree_status[by] = False
            await message.author.send("[채팅수집-동의] {}님의 채팅내역 수집 동의 거부가 정상적으로 처리되었습니다.".format(name))

    async def on_message(self, message: discord.Message) -> bool:
        self.logger.info("[{0.author}][{author_id}] {0.content}".format(message, author_id=message.author.id))
        return False
